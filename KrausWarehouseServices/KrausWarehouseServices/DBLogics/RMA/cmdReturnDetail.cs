   ...
   public class cmdReturnDetail
    {
       ...
       public Boolean UpsertReturnDetail(ReturnDetailsDTO ReturnsDetail)
       {
           ...
           try
           {
               ...
               if (_ReturnDetail == null)
               {
                   ...
                   _ReturnDetail.ProductID = ReturnsDetail.ProductID;
                   _ReturnDetail.SalesPrice = ReturnsDetail.SalesPrice;

                   _ReturnDetail.LineType = ReturnsDetail.LineType;

                   _ReturnDetail.ShipmentLines = ReturnsDetail.ShipmentLines;
                   _ReturnDetail.ReturnLines = ReturnsDetail.ReturnLines;

                   _ReturnDetail.TrackingNumber=ReturnsDetail.TrackingNumber;
       ...
       public List<ReturnDetails> GetTrackingNumbersInReturnDetails()
       {
           ...
           try
           {
               var lst = (from _returnfdetail in entRMA.ReturnDetails
                          where _returnfdetail.TrackingNumber != ""
                          select new
                          {
                              _returnfdetail.ReturnID,
                              _returnfdetail.TrackingNumber,

                          }).Distinct();

               //List<ReturnDetailsDTO> k = entRMA.ReturnDetails.Select(x => x.).Distinct().ToList();

               //_lsreturn = lst.ToString().ToList();
               foreach (var item in lst)
               {
                   ReturnDetails returndetail = new ReturnDetails();
                   returndetail.TrackingNumber = item.TrackingNumber;
                   returndetail.ReturnID = item.ReturnID;
                   _lsreturn.Add(returndetail);
               }
           }
           catch (Exception)
       ...
       public Boolean DeleteBothReasonsForeignKeyTables(Guid ReturnDetailsID)
       {
           Boolean _returnFlag = false;
           try
           {
       ...
       public Boolean DeleteRecord(Guid ReturnDetailsID)
       {
           Boolean _returnFlag = false;
           try
           {
               ...
               List<SKUReason> SkuReasons = (from Ls in entRMA.SKUReasons
                                             where Ls.ReturnDetailID == ReturnDetailsID
                                             select Ls).ToList();
               ...
               //}
               //entRMA.SaveChanges();
               foreach (SKUReason skupointsitem in SkuReasons)
               {
                   entRMA.Attach(skupointsitem);
                   entRMA.DeleteObject(skupointsitem);
               }
