   ...
   public  class cmdReturnImages
    {
       ...
       public Boolean DeletePathImageS(Guid ReturnDetailID, string skuimgPath)
       {
           ...
           try
           {
               ...
               string path = @"\\192.168.1.172\Macintosh HD\ftp_share\RGAImages\" + skuimgPath;

               // path = entRMA.ReturnImages.SingleOrDefault(r => r.ReturnDetailID == ReturnImageID).ToString();

               var id = (from img in entRMA.ReturnImages
                         where img.ReturnDetailID == ReturnDetailID
                         where img.SKUImagePath == path
                         select img).First();

               entRMA.ReturnImages.DeleteObject(id);
               entRMA.SaveChanges();
