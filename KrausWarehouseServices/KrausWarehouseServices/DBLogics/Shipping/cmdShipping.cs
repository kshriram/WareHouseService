   ...
   public class cmdShipping
    {
     ...
     #region Get Methods For Shipping
       ...
       public List<ShippingDTO> GetAllShipmentsWithPackage()
       {
           List<ShippingDTO> _ShipInfo = new List<ShippingDTO>();

           try
           {

               var shipment = entShipping.ExecuteStoreQuery<ShippingDTO>(@"select
                                                                                    S.ShippingNum, S.ShippingStartTime, S.DeliveryProvider, S.DeliveryMode, S.OrderID, S.CustomerPO, S.Carrier, S.VendorName,
                                                                                    CONVERT(VARCHAR(10), S.ShippingStartTime, 101) StartDate                                                                           
                                                                                    from
                                                                                    Shipping S left join Package P on S.ShippingID = P.ShippingID
                                                                                    where S.ShippingNum = P.ShippingNum
                                                                                    order by S.ShippingID").ToList();


               //foreach (var item in shipment)
               //{
               //    ShippingDTO _ShipmentInfo = new ShippingDTO();
               //    _ShipmentInfo.ShippingNum = item.ShippingNum;
               //    _ShipmentInfo.ShippingStartTime = item.ShippingStartTime;
               //    _ShipmentInfo.DeliveryProvider = item.DeliveryProvider;
               //    _ShipmentInfo.DeliveryMode = item.DeliveryMode;
               //    _ShipmentInfo.OrderID = item.OrderID;
               //    _ShipmentInfo.CustomerPO = item.CustomerPO;
               //    _ShipmentInfo.Carrier = item.Carrier;
               //    _ShipmentInfo.VendorName = item.VendorName;
