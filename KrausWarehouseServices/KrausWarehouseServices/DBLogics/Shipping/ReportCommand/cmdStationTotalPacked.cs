...
namespace KrausWarehouseServices.DBLogics.Shipping.ReportCommand
{
    public class cmdStationTotalPacked
    {
        ...
        public List<UserTotalPacked> GetEachUserPacked(DateTime DateReport)
        {
            List<UserTotalPacked> _lsUserPacked = new List<UserTotalPacked>();
            try
            {
                var vStationPacked = from user in lent.Users
                                     join pack in lent.Packages
                                     on user.UserID equals pack.UserId

                                     where EntityFunctions.TruncateTime(pack.StartTime) == EntityFunctions.TruncateTime(DateReport)

                                     group pack by user.UserID into GUserPack
                                     select new
                                     {
                                         UserID = GUserPack.Key,
                                         UserName = GUserPack.FirstOrDefault(i => i.User.UserName != null).User.UserFullName,
                                         PackedCount = GUserPack.Count(i => i.PackingStatus == 0),
                                         PartiallyPacked = GUserPack.Count(i => i.PackingStatus == 1)
                                     };

                foreach (var item in vStationPacked)
                {
                    UserTotalPacked _spacked = new UserTotalPacked();
                    _spacked.UserID = item.UserID;
                    _spacked.UserName = item.UserName;
                    _spacked.TotalPacked = item.PackedCount;
                    _spacked.PartiallyPacked = item.PartiallyPacked;
                    _lsUserPacked.Add(_spacked);
                }
            }
            catch (Exception)
            { }
            return _lsUserPacked;
